// August/2020 Updated by Jamin M-l <jakkle21@gmail.com>
// Updated to work with Mixxx 2.2+
//
...
NumarkMixTrackPro.loopIn2 = function(channel, control, value, status, group) {
	var deck = NumarkMixTrackPro.groupToDeck(group);
	...
	if (!value) return;
		...
		if (start<0 || end<0) {
//
// I need to fix the LED code here.
		...
		NumarkMixTrackPro.setLED(NumarkMixTrackPro.leds[deck]["loop_start_position"], true);		} else {
...
NumarkMixTrackPro.loopOut2 = function(channel, control, value, status, group) {
	var deck = NumarkMixTrackPro.groupToDeck(group);
...
NumarkMixTrackPro.reLoop = function(channel, control, value, status, group) {
	...
	if (NumarkMixTrackPro.manualLoop[deck-1]){
		...
		if (NumarkMixTrackPro.deleteKey[deck-1]){
			...
			engine.setValue(group, "reloop_toggle", status?1:0);
		}
		return;
	}
...
NumarkMixTrackPro.effectToggleKey = function(channel, control, value, status, group){
	if (!value) return;
		if (NumarkMixTrackPro.deleteKey[deck-1]){
//change slot
    } else {
...
NumarkMixTrackPro.effectSelectKnob = function(channel, control, value, status, group){
		if (NumarkMixTrackPro.deleteKey[deck-1]){
// change key
    } else {
// change effect in slot        
    }
}

NumarkMixTrackPro.effectunitToggle = function(channel, control, value, status, group){
		if (NumarkMixTrackPro.deleteKey[deck-1]){
//  reset key
    } else {
// enable unit ondeck        
